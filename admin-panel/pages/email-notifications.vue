  ...
  <div class="flex min-h-screen bg-slate-50 font-sans">
    ...
    <main class="flex-1 p-10 overflow-y-auto max-h-screen">
      <div class="max-w-2xl mx-auto">
        <header class="mb-8">
          <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Email Notifications</h1>
          <p class="text-slate-500 mt-1">Set the email address that receives order and inquiry alerts.</p>
        </header>
        ...
        <div v-else class="bg-white border border-slate-200/60 rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] p-8">
          <form @submit.prevent="save" class="flex flex-col gap-6">
            <div class="space-y-1.5">
              <label class="block text-sm font-semibold text-slate-700">Notification Email</label>
              <p class="text-xs text-slate-400">New orders and customer inquiries will be sent to this address.</p>
              <input v-model="notificationEmail" type="email" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-brand-lightblue focus:border-brand-lightblue transition-all outline-none" placeholder="info@example.com" />
            </div>
            ...
            <div class="flex items-center gap-4 pt-2">
              ...
              <span v-if="savedMessage" class="text-sm font-medium" :class="savedMessage.startsWith('Error') ? 'text-red-500' : 'text-green-600'">{{ savedMessage }}</span>
            </div>
          </form>
        </div>
      </div>
